...
object Main {
  ...
  def main(args: Array[String]) {
    ...
    def AggregateControlPlane(spark: SparkSession): Unit = {
      ...
      println(files)
      //val destinationFilename=files

      val destinationFilename = new File("/opt/IOVisor-Data/flows/").list.toList // gives list of file names including extensions in the path `path`
      println("Printing Source File names")
      println(destinationFilename)

      val out_path ="/opt/IOVisor-Data/flowsOut/"
      for (f <- destinationFilename) {
      ...
      */
      ...
      def getListFiles(dir: String): List[String] = {
        val sourcefile = new File(dir)
        sourcefile.listFiles.filter(_.isFile)
          .filter(_.getName.endsWith(".csv"))
          .map(_.getPath).toList
      }
      val source=getListFiles("/opt/Multi-View-Staged/AggregateControlPlane.csv")
      println("Printing Source File names")
      println(source)
      println(destinationFilename)

      // val sourceFilename = new File("/opt/Multi-View-Staged/AggregateControlPlane.csv").sourceFilename.listFiles.filter(_.isFile).filter(_.getName.startsWith("03_ss")).map(_.getPath).toListlist.toList // gives list of file names including extensions in the path `path`
      //println("Printing Source File names")
      //println(sourceFilename)

      val path = Files.move(
        Paths.get("/opt/Multi-View-Staged/AggregateControlPlane.csv/"+source),
        Paths.get("/opt/IOVisor-Data/flowsOut/"+destinationFilename+".csv"),
